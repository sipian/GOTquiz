...
require_once './checkLogin.php';
  if($variableToCheckLoggedIn == false) //1 if condition check if cookies tampered with
    ...
    //initialize the variables
        ...
        $sql = "select points , $questionDetail"."Count,$questionDetail"."Solved , section1 from scoreTable where username = \"".$_SESSION["username"]."\"";
        ...
        if(mysqli_num_rows($result) == 1){//3 if count == 1 for count & solved
          ...
          if($row["section1"] == 'yes')
           ...
           if($trialsLeft == 0 && $solved == 'no'){//"4 if chance & solved"
             ...
             $nextButton = '<a href="./dashboard.php"><img src="./images/next.jpg" id="next" alt="NEXT"/></a>';
           }
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST"){//5 if form submitted
              $answer = $_POST["answer"];
                   if(strcasecmp($answer,$solution) == 0){//8 if answer is correct
                    $answerStatistic = "Correct";
                    $sql = "update scoreTable SET ".$questionDetail."Count = ".$questionDetail."Count - 1 , ".$questionDetail."Solved = 'yes' , points = points + $PtsForSection1 , currentTime = NOW() where username = \"".$_SESSION["username"]."\"";
